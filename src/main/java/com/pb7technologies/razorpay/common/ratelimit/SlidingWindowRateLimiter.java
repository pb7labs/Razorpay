...
public class SlidingWindowRateLimiter implements RateLimiter {
    ...
    public RateLimitResult check(String key, int maxRequestAllowed, long windowSeconds) {
        ...
        if (current >= maxRequestAllowed) {
            var oldest = zset.rangeWithScores(redisKey, 0, 0); //First element is oldest
            int retryAfter = 1;

            if (oldest != null && !oldest.isEmpty()) {
                Double oldestScore = oldest.iterator().next().getScore(); //score gives time
                if (oldestScore != null) {
