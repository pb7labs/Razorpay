...
public class WebhookDeliverExecutor {
    ...
    @Value(("${webhook.deliver.signature-header:X-Razorpay-Signature}"))
    ...
    public void deliver(UUID webhookEventId) {
        ...
        if (webhookEvent.isEmpty()) {
            log.warn("No webhook event found for id= {}", webhookEventId);
            return;
        }
        ...
        try {
            var response = restClient.post()
                    ...
                    .header(signatureHeader, event.getSignature()) // decode the payload
                    .contentType(MediaType.APPLICATION_JSON)
                    .body(Map.of(
                            "event", event.getEventType(), "payload", event.getPayload()
                    )).retrieve()
                    .toBodilessEntity(); // body less entity we do not want to do anything after making request
